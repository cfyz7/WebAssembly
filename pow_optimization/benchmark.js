...
const fs = require('fs');
const bytes = fs.readFileSync('./pow2.wasm');
...
function init_benchmark() {
  ...
  suite.add('#2 '.yellow + 'Обратный порядок Div/Mul', pow2_reverse);
  suite.add('#3 '.yellow + 'Замена умножения сдвигом', pow2_mul_shift);
  suite.add('#4 '.yellow + 'Замена умножения и деления сдвигом', pow2_mul_div_shift);
  suite.add('#5 '.yellow + 'Версия оптимизированная wasm-opt', pow2_opt);
  suite.add('#6 '.yellow + 'Использование сдвига в исходном порядке', pow2_mul_div_nor);
	// add listeners
	...
	suite.on('complete', function () {
		...
		let fast_string = ('Самый быстрый ' + this.filter('fastest').map('name'));
		let slow_string = ('Самый медленный ' + this.filter('slowest').map('name'));
		console.log(`
		------------------------------------------------------------
			${fast_string.green}
			${slow_string.red}
