...
;; Функция которая задаёт каждому пикселю цвет
  (func $set_pixel
    (param $x i32)
    (param $y i32)
    (param $c i32)
...
;; Рисует квадрат пикселей через функцию $set_pixel
  (func $draw_obj
    (param $x i32)
    (param $y i32)
    (param $c i32)
...
;; Размещение обьекта в памяти исходя из значения шага 16 байт
  (func $set_obj_attr
...
;; Получение обьекта в памяти на основе значения шага
  (func $get_obj_attr
    (param $obj_number i32)
    (param $attr_offset i32)
    (result i32)
